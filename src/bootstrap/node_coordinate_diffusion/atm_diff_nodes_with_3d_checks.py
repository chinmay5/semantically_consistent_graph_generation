...
from src.ConStruct.datasets.atm_dataset import ATMGraphDataset
...
# Some Globals
COORDINATE_SIZE = 3  # Can be 2 or 3
...
class NodeCoordsDiffusionModel:
    ...
    def sample(self, pos, node_mask, save_intermediate=False, pc_save_dir=None):
        self.model.eval()
        with torch.no_grad():
            # The last dimension has an extra column for the weighting factor.
            # Hence, we simply use the already known information about the coordinate size
            x = torch.randn((pos.shape[0], pos.shape[1], COORDINATE_SIZE)).to(pos.device)
            for i in reversed(range(1, self.noise_steps)):
                ...
                x = 1 / torch.sqrt(alpha) * (
                    ...
                    beta) * noise
                if save_intermediate and (i % 50 == 0 or i == 1):
                    # Iterate over each of the batch dimensions
                    pc_t = x[0][node_mask[0]]
                    pc = (pc_t - torch.sqrt(1 - alpha_hat) * predicted_noise[0][node_mask[0]]) / torch.sqrt(alpha_hat)
                    cloud = pyvista.PolyData(pc.view(-1, 3).cpu().numpy())
                    # Save as a VTP file
                    cloud.save(os.path.join(pc_save_dir, f"intermediate_sample_{i}.vtp"))
        self.model.train()
        return x
...
def perform_coord_wise_kde(coords_synth, coords_real, epoch):
    ...
    kde_plot_dir = os.path.join(PROJECT_ROOT_DIR, 'bootstrap', 'node_coordinate_diffusion', 'atm_kde_plots')
...
def get_data(args):
    data_root = os.path.join(PROJECT_ROOT_DIR, 'data', 'atm', 'pt_files')
    train_dataset = ATMGraphDataset(dataset_name='atm', split='train', root=data_root, node_jitter=False)
    val_dataset = ATMGraphDataset(dataset_name='atm', split="val", root=data_root, node_jitter=False)
...
def batched_icp_mapping(pred_pc_batch, gt_pc_batch, padding_mask):
    B, N, _ = pred_pc_batch.shape
    _, M, _ = gt_pc_batch.shape
    device = pred_pc_batch.device

    aligned_pred_batch = torch.zeros_like(pred_pc_batch)
    mapping_indices = torch.full((B, N), -1, dtype=torch.long, device=device)

    for b in range(B):
        valid_pred = pred_pc_batch[b][padding_mask[b] == 1].detach().cpu().numpy()
        valid_gt = gt_pc_batch[b][padding_mask[b] == 1].detach().cpu().numpy()

        pred_pc_o3d = o3d.geometry.PointCloud()
        pred_pc_o3d.points = o3d.utility.Vector3dVector(valid_pred)

        gt_pc_o3d = o3d.geometry.PointCloud()
        gt_pc_o3d.points = o3d.utility.Vector3dVector(valid_gt)

        threshold = 0.02
        icp_result = o3d.pipelines.registration.registration_icp(
            pred_pc_o3d, gt_pc_o3d, threshold, np.eye(4),
            o3d.pipelines.registration.TransformationEstimationPointToPoint()
        )

        transformed_pred = np.asarray(pred_pc_o3d.transform(icp_result.transformation).points)

        transformed_pred_tensor = torch.tensor(transformed_pred, dtype=torch.float32, device=device)
        gt_tensor = torch.tensor(valid_gt, dtype=torch.float32, device=device)

        distances = torch.cdist(transformed_pred_tensor, gt_tensor)
        nearest_indices = torch.argmin(distances, dim=1)

        aligned_pred_batch[b][padding_mask[b] == 1] = transformed_pred_tensor
        mapping_indices[b][padding_mask[b] == 1] = nearest_indices

    return aligned_pred_batch, mapping_indices
...
def train(args):
    ...
    for epoch in pbar:
        ...
        for i, data in enumerate(train_dataloader):
            ...
            loss.backward()
            optimizer.step()
            logger.add_scalar("MSE", loss.item(), global_step=(epoch * args.batch_size + i))
            epoch_loss += loss.item()
            ctr += 1
        if epoch % 100 == 99:
            pc_save_dir = os.path.join(PROJECT_ROOT_DIR, 'bootstrap', 'node_coordinate_diffusion', 'atm_sample_pcs',
                                       'val', f'epoch_{epoch}')
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(val_loader):
                    ...
                    sampled_pos_batched = diffusion.sample(pos, node_mask)
                    sampled_pos = sampled_pos_batched[node_mask]
                    ...
                    for x in range(sampled_pos_batched.shape[0]):
                        # Iterate over each of the batch dimensions
                        pc = sampled_pos_batched[x][node_mask[x]]
                        cloud = pyvista.PolyData(pc.view(-1, 3).cpu().numpy())
                        sample_idx = i * args.batch_size + x
                        # Save as a VTP file
                        cloud.save(os.path.join(pc_save_dir, f"sample{sample_idx}.vtp"))
...
def launch():
    ...
    args.run_name = "DDPM_ATM_Uncondtional"
    args.epochs = 2000
    args.batch_size = 64
