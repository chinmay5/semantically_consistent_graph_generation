...
def perform_coord_wise_kde(folders, axes, min_start, max_start):
    ...
    for idx, folder in enumerate(folders):
        ...
        axes[0, 0].plot(pts, x_coord_kde(pts), label=f"KDE_x_coords_{labels[idx]}", color=colors[idx], alpha=max(0.5 ** idx, 0.5))
        axes[0, 1].plot(pts, y_coord_kde(pts), label=f"KDE_y_coords_{labels[idx]}", color=colors[idx], alpha=max(0.5 ** idx, 0.5))
        axes[0, 2].plot(pts, z_coord_kde(pts), label=f"KDE_z_coords_{labels[idx]}", color=colors[idx], alpha=max(0.5 ** idx, 0.5))
...
def plot_edge_length_kde(folders, axes):
    ...
    for idx, folder in enumerate(folders):
        ...
        small_edge_proportion = sum([x for x in edge_length_array if x <= 1 / 32]) / len(edge_length_array)
        # print(f"small edge proportion = {small_edge_proportion * 100}")
        if idx == 0:
    ...
    print(f"kl divergence between edge lengths baseline {kl_divergence_between_lists(edge_length_arr[0], edge_length_arr[1])}")
    print(f"kl divergence between edge lengths ours {kl_divergence_between_lists(edge_length_arr[0], edge_length_arr[2])}")
...
def betti_0_and_1_plots(folders, axes):
    ...
    print(
        f"Betti 0 KL divergence baseline {kl_divergence_between_lists(betti_0_real, betti_0_baseline, num_bins=5, bin_min=0.5, bin_max=5.5)}")
    print(
    ...
    print(
        f"Betti 0 KL divergence ours {kl_divergence_between_lists(betti_0_real, betti_0_ours, num_bins=5, bin_min=0.5, bin_max=5.5)}")
    print(
...
def plot_node_degree_staircase(folders, axes):
    ...
    print(f"kl divergence between node degrees baseline {kl_divergence_between_lists(list_of_dataset_degree[0], list_of_dataset_degree[1])}")
    print(f"kl divergence between node ours {kl_divergence_between_lists(list_of_dataset_degree[0], list_of_dataset_degree[2])}")
...
def cow_stats():
    ...
    real_folder = '/mnt/elephant/chinmay/COWN_plus_top_CoW/crown_vtps_camera_ready'
    baseline = '/home/chinmayp/workspace/ConStruct/outputs/2025-02-23/midi_baseline'
    # ours = '/home/chinmayp/workspace/ConStruct/outputs/2025-02-23/construct/generated_samples'
    # ours = '/home/chinmayp/workspace/ConStruct/outputs/2025-02-23/construct_plus/generated_samples'
    ours = '/home/chinmayp/workspace/ConStruct/outputs/2025-02-23/ours/generated_samples'
    # We compare the results
    fig, axes = plt.subplots(2, 3, figsize=(16, 8))
    # perform_coord_wise_kde(folders=[real_folder, baseline, ours], axes=axes, min_start=-1, max_start=1)
    # perform_node_degree_kde(folders=[real_folder, baseline, ours], axes=axes)
    # plot_num_edges_kde(folders=[real_folder, baseline, ours], axes=axes)
    # plot_edge_angles_along_axes_kde(folders=[real_folder, baseline, ours], axes=axes)
    ...
    plt.tight_layout()
    # plt.savefig('crown.jpg', format='jpg', dpi=250)
    plt.show()
