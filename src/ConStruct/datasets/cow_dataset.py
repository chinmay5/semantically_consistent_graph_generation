...
def load_cow_file(graph_filename):
    """
...
class CoWGraphDataset(Dataset):
    def __init__(self, dataset_name, split, root, node_jitter=True, transform=None, use_radius=False,
                 pre_transform=None,
                 pre_filter=None):
        ...
        self.crown_file_path = '/mnt/elephant/chinmay/COWN_plus_top_CoW/crown_vtps_camera_ready'
        self.preprocssed_graph_save_dir = os.path.join(PROJECT_ROOT_DIR, 'data', 'cow', base_load_dir)
        self.raw_dir = os.path.join(PROJECT_ROOT_DIR, 'data', 'cow', f'raw_{base_load_dir}')
        self.processed_dir = os.path.join(PROJECT_ROOT_DIR, 'data', 'cow', f'processed')
        ...
        stats_path = os.path.join(PROJECT_ROOT_DIR, 'data', 'cow', f'statistics_{base_load_dir}', f"{split}.pkl")
    ...
    def download(self):
        ...
        all_graph_filenames = []
        for base, _, filename_list in os.walk(self.crown_file_path):
            for filename in filename_list:
                if filename.endswith('multi_met_graph.vtp'):
                    all_graph_filenames.append(os.path.join(self.crown_file_path, base, filename))
        ...
        if num_graphs == 1:
            ...
            graph_data = load_cow_file(all_graph_filenames[0])
            ...
            for i, graph_filename in enumerate(all_graph_filenames):
                ...
                graph_data = load_cow_file(graph_filename)
                graph_data.y = torch.zeros((1, 0), dtype=torch.float)
...
class CoWDataModule(AbstractDataModule):
    def __init__(self, cfg):
        ...
        train_dataset = CoWGraphDataset(dataset_name=self.cfg.dataset.name,
                                        split='train', root=cfg.dataset.datadir,
                                        use_radius=cfg.dataset.use_radius)
        val_dataset = CoWGraphDataset(dataset_name=self.cfg.dataset.name,
                                      split='val', root=cfg.dataset.datadir,
                                      use_radius=cfg.dataset.use_radius)
        test_dataset = CoWGraphDataset(dataset_name=self.cfg.dataset.name,
...
class CoWDatasetInfos(AbstractDatasetInfos):
    ...
    def to_one_hot(self, pl):
        # Need to override specifically for the cow dataset.
        ...
        # Dimension matching for radius information
        if pl.radius is not None:
            pl.radius = pl.radius.unsqueeze(-1)
...
def compute_min_max_pos(split):
    ...
    dataset = CoWGraphDataset(dataset_name='sample', split=split, root=save_path)
...
def get_edge_attr_counts():
    ...
    cfg.use_aug = True
    save_path = os.path.join(PROJECT_ROOT_DIR, 'data', 'cow', 'pt_files')
    dataset = CoWGraphDataset(dataset_name='sample', split='train', root=save_path)
    all_attr = []
...
if __name__ == '__main__':
    save_path = os.path.join(PROJECT_ROOT_DIR, 'data', 'cow', 'pt_files')
    dataset = CoWGraphDataset(dataset_name='sample', split='test', root=save_path)
    check_deg_c_N()
    ...
    #     # datasets = CoWGraphDataset(dataset_name='sample', split='train', root=save_path)
    #     print(datasets[0])
    #     print(datasets[0].y)
