...
def compute_distances(points, sigma=1.0):
    ...
    rbf_matrix = torch.linalg.norm(coord_diff, dim=-1)  # B, N, N
    # rbf_matrix = torch.exp(-square_distances / (2 * sigma ** 2))

    return rbf_matrix.unsqueeze(-1)  # B, N, N, 1
...
class NodeEdgeBlock(nn.Module):
    ...
    def __init__(self, dx, de, dy, n_head, predicts_y=False):
        ...
        # Distance encoding
        # self.lin_dist_coords = Linear(1, de)  # The distance RBF encoding
    ...
    def forward(self, X, E, y, pos, node_mask):
        ...
        # pair_coords = compute_distances(points=pos)  # bs, n, n, 1
...
class Graph3DTransformer(nn.Module):
    """
    ...
    def forward(self, model_input: utils.PlaceHolder):
        ...
        features = utils.PlaceHolder(X=self.mlp_in_pos_high_dim(pos), E=new_E, y=self.mlp_in_y(y), charges=None,
