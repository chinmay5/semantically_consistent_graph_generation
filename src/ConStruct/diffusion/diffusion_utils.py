...
def sample_discrete_features(probX, probE, prob_charges, node_mask, top_k=None):
    """Sample features from multinomial distribution with given probabilities (probX, probE, proby)
    :param probX: bs, n, dx_out        node features
    :param probE: bs, n, n, de_out     edge features
    :param proby: bs, dy_out           global features.
    :param top_k: int                  number of top-k edges to sample
    """
    ...
    # We also sample top-k edges and just carry it along as extra information
    # This is not used by the baseline model and we use it only when
    # we want to correct the generated samples.
    # TODO: parameterize this
    if top_k is not None:
        E_t_multiclass = probE.multinomial(top_k).reshape(bs, n, n, top_k)
        # We make our lives simple. This is not the best way but, should work.
        # We get at most three possible candidates for each invalid edge.
        E_t_multiclass[..., 0] = E_t
        # sample_probs = torch.gather(probE, dim=1, index=E_t_multiclass)
        # shape: (B*n*n, 3)
        # Now, sort the probabilities in descending order along dimension 1.
        # sorted_probs, sort_order = torch.sort(sample_probs, dim=1, descending=True)
        # Reorder the samples using the sort order.
        # E_t_multiclass = torch.gather(E_t_multiclass, dim=1, index=sort_order)
        # Now, we reshape it back to the matrix shape.
        # E_t_multiclass = E_t_multiclass.reshape(bs, n, n, top_k)


        # The symmetrization is not so trivial here.
        # We need to permute the matrix and NOT apply the formula used above
        # Permute to shape (B, 3, N, N) so that the last two dims are the matrix dimensions.
        E_t_multiclass = E_t_multiclass.permute(0, 3, 1, 2)
        # Apply torch.triu to extract the upper triangular part along the last two dims.
        E_t_triu = torch.triu(E_t_multiclass, diagonal=1)
        # Make the matrix symmetric by adding its transpose (swap the last two dims).
        E_t_symm = E_t_triu + E_t_triu.transpose(-1, -2)
        # Permute back to the original shape (B, N, N, 3).
        E_t_multiclass = E_t_symm.permute(0, 2, 3, 1)

        # We take the first dimension of the top-k edges as E_t.
        # This is equivalent to taking the most probable edge.
        E_t = E_t_multiclass[..., 0]
    else:
